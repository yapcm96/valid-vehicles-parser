module Validator
  ...
  def self.validate_make(make:)
    unless VALID_MAKE.include? make.symbolize
      raise ValidationError.new, "Vehicle has make of '#{make}' which is not part of the parser's accepted vehicle makes (i.e. one of #{VALID_MAKE_STR})."
    end
  end

  def self.validate_colour(colour:)
    unless VALID_COLOUR.include? colour.symbolize
      raise ValidationError.new, "Vehicle has colour of '#{colour}' which is not part of the parser's accepted vehicle colours (i.e. one of #{VALID_COLOUR_STR})."
    end
  end
